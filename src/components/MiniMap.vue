...
export default {
  ...
  computed: mapState(["areaTitle", "roomTitle"]),
  watch: {
    minimapData(data) {
  ...
  methods: {
    ...
    renderMap() {
      ...
      const filteredRooms = this.minimapData.rooms.filter(
        (r) => r.z === this.characterLocation.z
      );
      filteredRooms.forEach((room) => {
        ...
        if (this.$store.state.permissions.indexOf("CAN_BUILD") >= 0) {
          ...
          sprite.on(
            "pointerover",
            () => (sprite.tint = this.rgbToHex("255,255,0"))
          );
          sprite.on(
            "pointerout",
        ...
        directions.forEach((dir) => {
          if (room[dir].length > 0) {
            if (
              dir === "up" ||
              dir === "down" ||
              room[dir].split(",")[0] !== this.areaTitle
            ) {
    ...
    centerMapOnLocation: function (location) {
      ...
      var x =
        ...
        -(location.x * gridSizeFull + this.gridPadding * location.x) -
        gridSizeFull / 2;
      var y =
        this.app.screen.height / 2 +
        (location.y * gridSizeFull + this.gridPadding * location.y) -
        gridSizeFull / 2;
      ease.add(
        this.mapContainer,
        { x: x, y: y },
        { duration: 100, repeat: false, reverse: false }
      );

      // Tint the background.
      this.backgroundTilingSprite.tint = this.rgbToHex(
        this.roomColor(location),
        -100
      );
    },
    ...
    roomColor: function (location) {
      for (
        let roomIdx = 0;
        roomIdx < this.minimapData.rooms.length;
        roomIdx++
      ) {
  ...
  mounted() {
    this.$store.dispatch("setAreaTitle", {
      data: this.$t("map.area.unknown-title"),
    });
    this.$store.dispatch("setRoomTitle", {
      data: this.$t("map.room.unknown-title"),
    });
